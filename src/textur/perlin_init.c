...
** Last update Sun Jun  8 20:28:56 2014 david tran
*/
...
int	init_func_perlin(t_rt *rt)
{
  ...
  if (!(rt->N2D.vals = (double*)
	malloc(sizeof(double) * (rt->N2D.high_max * rt->N2D.len_max + 3))))
    return (-1);
...
int		perlin_color(t_rt *rt, t_ray *ray, t_obj *obj)
{
