...
** Last update Sun Jun  8 23:21:51 2014 david tran
*/

#include "rt.h"

void	init_ray(t_rt *rt, t_ray *ray, int x, int y)
{
  if (!ray || !rt)
    return;
  ray->x = x;
...
void	assign_ray(t_rt *rt, t_ray *ray, t_obj *obj)
{
  if (!rt || !ray || !obj)
    return ;
...
colour		get_the_scene(t_rt *rt, int x, int y)
{
  t_ray		ray;
  t_color	color;
  t_obj		*tmp;
